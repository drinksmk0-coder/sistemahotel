-- Salva cada acompanhante preenchido no check-in online como hóspede real da reserva.
...
begin
  ...
  begin
    v_companions := coalesce(nullif(p_form_data ->> 'acompanhantes_detalhes', '')::jsonb, '[]'::jsonb);
  exception when others then
    raise exception using errcode = '22023', message = 'A lista de acompanhantes está inválida.';
  end;
  ...
  if found then
    if v_reservation.cliente_id is not null then
      update public.clients
         set nome = v_name,
             ...
             cpf = case when upper(coalesce(p_form_data ->> 'tipo_documento', 'CPF')) = 'CPF' then coalesce(v_document, cpf) else cpf end,
    ...
    insert into public.reservation_guests (
      ...
      v_name,
      case when upper(coalesce(p_form_data ->> 'tipo_documento', 'CPF')) = 'CPF' then v_document else null end,
      v_phone,
      v_email,
      v_birth,
    ...
    for v_companion in select value from jsonb_array_elements(v_companions)
    loop
      v_companion_name := nullif(trim(v_companion ->> 'nome'), '');
      if v_companion_name is null then
        raise exception using errcode = '22023', message = 'Todo acompanhante deve ter nome completo.';
      end if;
      ...
      insert into public.reservation_guests (
        ...
        v_reservation.id,
        v_companion_name,
        nullif(trim(v_companion ->> 'cpf'), ''),
        nullif(trim(v_companion ->> 'telefone'), ''),
        nullif(trim(v_companion ->> 'email'), ''),
        nullif(v_companion ->> 'data_nascimento', '')::date,
        nullif(trim(v_companion ->> 'sexo'), ''),
        coalesce(nullif(trim(v_companion ->> 'parentesco'), ''), nullif(trim(v_companion ->> 'tipo'), ''), 'acompanhante'),
        false
      );
