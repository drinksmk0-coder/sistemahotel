-- Protege o fluxo público da FNRH sem apagar fichas existentes.
-- Links pendentes expiram em 72h. Fichas enviadas não podem ser reenviadas.
-- O acesso público pré-envio recebe somente dados mínimos; ficha completa fica restrita
-- a dono/recepção, mantendo uma janela curta de 10 minutos para a impressão atual.
...
set expires_at = case
  when status = 'enviado' then coalesce(created_at, now()) + interval '72 hours'
  else coalesce(submitted_at, updated_at, now())
end
...
create index if not exists guest_checkins_public_access_idx
  on public.guest_checkins (public_token, status, expires_at);
...
begin
  ...
  v_recent_print := v_gc.status = 'preenchido'
                    and v_gc.submitted_at is not null
                    and v_gc.submitted_at > now() - interval '10 minutes';
  v_full_access := v_is_staff or v_recent_print;

  if not v_is_staff and not v_public_open and not v_recent_print then
    return null;
  end if;
  ...
  return jsonb_build_object(
    ...
    'checkin', v_reservation.checkout - (v_reservation.checkout - v_reservation.checkin),
...
begin
  ...
  return jsonb_build_object(
    ...
    'print_available_until', v_checkin.submitted_at + interval '10 minutes'
  );
end;
$$;
...
grant execute on function public.submit_guest_checkin(uuid, jsonb, text, boolean) to anon, authenticated, service_role;
