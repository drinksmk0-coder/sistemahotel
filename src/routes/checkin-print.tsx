...
function CheckinPrint() {
  ...
  return <main className="min-h-screen bg-neutral-100 py-5 print:bg-white print:py-0">
    ...
    <article className="print-sheet mx-auto min-h-[297mm] w-full max-w-[210mm] bg-white px-[9mm] py-[8mm] text-[#1f2f46] shadow-xl">
      <header className="print-section border-b-2 border-[#243b5a] pb-3">
        <div className="grid grid-cols-[68px_1fr] gap-3">
          <img src="/hotel-real-logo.png" alt={data.company_name} className="h-[60px] w-[68px] object-contain" />
          <div><h1 className="text-center text-[17px] font-black uppercase tracking-tight">Ficha Nacional de Registro de Hóspedes</h1>
            <div className="mt-2 grid grid-cols-2 gap-x-4 text-[8.5px] leading-4">
      ...
      <Section title={`Acompanhantes (${companions.length})`}>
        {companions.length === 0 ? <div className="rounded border border-[#9aa8b8] px-2 py-2 text-[9px]">Reserva somente para o titular.</div> : <div className="space-y-2">{companions.map((guest, index) => <div key={`${guest.nome}-${index}`} className="companion-card grid grid-cols-4 gap-1.5 rounded border border-[#9aa8b8] p-2">
      ...
      <section className="signature-block mt-4 grid grid-cols-[1fr_78mm] items-end gap-5">
        <div className="text-[8px] leading-4 text-neutral-600"><p><strong>Declaração:</strong> o hóspede declara que as informações fornecidas são verdadeiras e autoriza seu tratamento para reserva, hospedagem, identificação, segurança, FNRH e cumprimento de obrigações legais.</p><p className="mt-1"><strong>Proteção de dados:</strong> acesso restrito a pessoas autorizadas e tratamento conforme a LGPD e as finalidades informadas.</p></div>
        <div><div className="flex h-[28mm] items-center justify-center border-b border-[#243b5a]">{data.signature_data_url ? <img src={data.signature_data_url} alt="Assinatura do hóspede" className="max-h-[23mm] max-w-[72mm] object-contain" /> : <span className="text-[9px] text-neutral-500">Assinatura não encontrada</span>}</div><p className="pt-1 text-center text-[9px] font-semibold">Assinatura do hóspede titular</p></div>
      </section>
    </article>
  </main>;
}

function Section({ title, children }: { title: string; children: React.ReactNode }) { return <section className="print-section mt-3"><h2 className="mb-1.5 border-b border-[#cbd4df] pb-1 text-[11px] font-black uppercase tracking-wide">{title}</h2>{children}</section>; }
function Field({ label, value, className = "" }: { label: string; value: string; className?: string }) { return <div className={`print-row ${className}`}><p className="mb-0.5 text-[7px] font-bold uppercase tracking-wide text-[#52657b]">{label}</p><div className="min-h-[21px] rounded border border-[#9aa8b8] px-1.5 py-1 text-[8.5px] leading-3.5">{value || "Não informado"}</div></div>; }
