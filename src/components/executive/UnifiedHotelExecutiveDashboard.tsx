...
type Sale = { data: string; total: number; comprador_tipo: string | null; status: string | null };
...
type SourceBundle = { daily: Daily[]; revenueSources: RevenueSource[]; reservations: Reservation[]; sales: Sale[]; expenses: Expense[]; rooms: Room[]; forecast: Forecast[] };
...
export function UnifiedHotelExecutiveDashboard() {
  ...
  const source = useQuery({
    ...
    queryFn: async (): Promise<SourceBundle> => {
      const [daily, revenueSources, reservations, sales, expenses, rooms, forecast] = await Promise.all([
        (supabase as any).from("bi_dashboard_diario").select("data,quartos_ocupados,quartos_total,ocupacao_pct,receita_hospedagem,receita_extras,receita_total,despesas,gop,adr,revpar,cancelamentos,no_shows").eq("company_id", companyId).lte("data", today).order("data"),
        (supabase as any).from("bi_receita_origem_diaria").select("data,origem_receita,receita").eq("company_id", companyId).lte("data", today).order("data"),
      ...
      return {
        daily: (daily.data ?? []) as Daily[], revenueSources: (revenueSources.data ?? []) as RevenueSource[], reservations: (reservations.data ?? []) as Reservation[],
        sales: (sales.data ?? []) as Sale[], expenses: (expenses.data ?? []) as Expense[], rooms: (rooms.data ?? []) as Room[],
  ...
  const model = useMemo(() => {
    ...
    const prevEndDate = new Date(`${start}T12:00:00`); prevEndDate.setDate(prevEndDate.getDate() - 1);
    const prevEnd = prevEndDate.toISOString().slice(0, 10);
    const prevStartDate = new Date(`${prevEnd}T12:00:00`); prevStartDate.setDate(prevStartDate.getDate() - selectedDays + 1);
    ...
    for (const s of selectedSales) { const row = dailyMap.get(s.data) ?? { data: s.data, hospedagem: 0, produtos: 0, despesas: 0 }; row.produtos += num(s.total); dailyMap.set(s.data, row); }
    const revenueEvolution = aggregateTimeline([...dailyMap.values()], start, end);

    const actualOccupancy = aggregateOccupancy(selectedDaily, start, end);
    const forecastRows = data.forecast.filter((f) => f.date > today && f.date <= end).map((f) => ({ iso: f.date, label: brDate(f.date), real: null as number | null, previsto: num(f.expected_occupancy), confirmados: num(f.confirmed_rooms) }));
    const occupancyTimeline = [...actualOccupancy.map((r) => ({ iso: r.iso, label: r.label, real: r.value, previsto: null as number | null, confirmados: null as number | null })), ...forecastRows].sort((a, b) => a.iso.localeCompare(b.iso));
    ...
    for (const r of filteredReservations) {
      ...
      x.reservas += 1; if (r.status !== "cancelado") x.receita += num(r.valor_total); if (r.elegivel_cancelamento_pre_checkin) { x.eligible += 1; x.canc += num(r.cancelado_flag); } channelMap.set(r.canal_analitico, x);
    }
    const channelPerformance = [...channelMap].map(([canal, x]) => ({ canal, receita: x.receita, reservas: x.reservas, cancelamento: x.eligible ? x.canc / x.eligible * 100 : null })).sort((a, b) => b.receita - a.receita);

    const profileMap = new Map<string, number>(), ageMap = new Map<string, number>(), childMap = new Map<string, number>();
    for (const r of valid) {
      profileMap.set(r.perfil_familiar, (profileMap.get(r.perfil_familiar) ?? 0) + 1); ageMap.set(r.faixa_idade, (ageMap.get(r.faixa_idade) ?? 0) + 1);
      const k = r.quantidade_filhos <= 0 ? "Sem filhos informados" : r.quantidade_filhos === 1 ? "1 filho" : "2+ filhos"; childMap.set(k, (childMap.get(k) ?? 0) + 1);
    }
    ...
    const productSegments = ["hospede", "funcionario", "empresa", "outro"].map((kind) => {
      ...
      return { name: buyerLabel(kind), compras: values.length, total: values.reduce((a,b)=>a+b,0), mediana: median(values), mediaAparada: trimmedMean(values, .1) };
    ...
    return {
      kpis: { totalRevenue, occupancy, adr, revpar, trevpar, goppar, deltas: { revenue: delta(totalRevenue, prevRevenue), occupancy: occupancy - prevOccupancy, adr: delta(adr, prevAdr), revpar: delta(revpar, prevRevpar), trevpar: delta(trevpar, prevTrevpar) } },
      hospitalityRevenue, extrasRevenue, recordedExpenses, actualGop, projectedExpenses, projectedGop, revenueBySource, revenueEvolution, occupancyTimeline, channelPerformance,
      profiles, ages, children, productSegments, topRooms, lowRooms, cancellationRate, cancellationEligible: eligible.length,
      ageCoverage: valid.length ? valid.filter(r=>r.idade != null).length/valid.length*100 : 0, selectedDays, validReservations: valid.length, prevValidReservations: prevValid.length,
    };
  ...
  return (
    <main className="space-y-3 px-2 pb-8 sm:px-3" data-unified-executive-dashboard>
      <header className="relative flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-border bg-card p-3 shadow-sm">
        ...
        <div className="flex items-center gap-2"><span className="rounded-full border border-border bg-background px-3 py-2 text-[11px] font-bold text-muted-foreground">{brDate(start)} — {brDate(end)}</span><button type="button" onClick={() => setFiltersOpen(v=>!v)} className="relative inline-flex h-9 items-center gap-1.5 rounded-full border border-border bg-background px-3 text-xs font-extrabold text-foreground shadow-sm hover:bg-muted"><Filter className="h-3.5 w-3.5" /> Filtros</button></div>
        {filtersOpen && <div className="absolute right-3 top-14 z-30 w-[min(92vw,340px)] rounded-2xl border border-border bg-card p-3 shadow-2xl"><div className="grid grid-cols-2 gap-2">
      ...
      <section className="grid grid-cols-2 gap-2 md:grid-cols-3 xl:grid-cols-6">
        <Kpi title="Receita" value={fmtBRL(model.kpis.totalRevenue)} delta={model.kpis.deltas.revenue} /><Kpi title="Ocupação" value={pct(model.kpis.occupancy)} delta={model.kpis.deltas.occupancy} points /><Kpi title="ADR" value={fmtBRL(model.kpis.adr)} delta={model.kpis.deltas.adr} /><Kpi title="RevPAR" value={fmtBRL(model.kpis.revpar)} delta={model.kpis.deltas.revpar} /><Kpi title="TRevPAR" value={fmtBRL(model.kpis.trevpar)} delta={model.kpis.deltas.trevpar} /><Kpi title="GOPPAR registrado" value={fmtBRL(model.kpis.goppar)} />
      </section>

      <section className="grid gap-3 xl:grid-cols-2">
        <ChartCard title="Receita por dia e mês"><ResponsiveContainer width="100%" height={280}><ComposedChart data={model.revenueEvolution}><CartesianGrid stroke={CHART.grid} strokeDasharray="3 3" vertical={false}/><XAxis dataKey="label" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}}/><Tooltip formatter={(v:any)=>fmtBRL(num(v))}/><Legend/><Bar dataKey="hospedagem" name="Hospedagem" stackId="r" fill={CHART.navy}/><Bar dataKey="produtos" name="Produtos" stackId="r" fill={CHART.teal}/><Line type="monotone" dataKey="total" name="Receita total" stroke={CHART.green} strokeWidth={3}/></ComposedChart></ResponsiveContainer></ChartCard>
        <ChartCard title="De onde vem a receita"><ResponsiveContainer width="100%" height={280}><BarChart data={model.revenueBySource} layout="vertical" margin={{left:24,right:20}}><CartesianGrid stroke={CHART.grid} strokeDasharray="3 3" horizontal={false}/><XAxis type="number" tick={{fontSize:10}}/><YAxis type="category" dataKey="name" width={132} tick={{fontSize:10}}/><Tooltip formatter={(v:any)=>fmtBRL(num(v))}/><Bar dataKey="value" name="Receita" fill={CHART.navy}/></BarChart></ResponsiveContainer></ChartCard>
      </section>

      <section className="grid gap-3 xl:grid-cols-2">
        <ChartCard title="Ocupação real e previsão"><ResponsiveContainer width="100%" height={280}><LineChart data={model.occupancyTimeline}><CartesianGrid stroke={CHART.grid} strokeDasharray="3 3" vertical={false}/><XAxis dataKey="label" tick={{fontSize:10}}/><YAxis domain={[0,100]} unit="%" tick={{fontSize:10}}/><Tooltip/><Legend/><Line type="monotone" dataKey="real" name="Real" stroke={CHART.navy} strokeWidth={3} connectNulls={false}/><Line type="monotone" dataKey="previsto" name="Previsto" stroke={CHART.sky} strokeWidth={3} strokeDasharray="5 4" connectNulls={false}/></LineChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Receita e cancelamento por canal"><ResponsiveContainer width="100%" height={280}><ComposedChart data={model.channelPerformance}><CartesianGrid stroke={CHART.grid} strokeDasharray="3 3" vertical={false}/><XAxis dataKey="canal" tick={{fontSize:10}}/><YAxis yAxisId="money" tick={{fontSize:10}}/><YAxis yAxisId="pct" orientation="right" domain={[0,100]} unit="%" tick={{fontSize:10}}/><Tooltip formatter={(v:any,n:any)=>n==="Cancelamento"?pct(num(v)):fmtBRL(num(v))}/><Legend/><Bar yAxisId="money" dataKey="receita" name="Receita" fill={CHART.navy}/><Line yAxisId="pct" type="monotone" dataKey="cancelamento" name="Cancelamento" stroke={CHART.red} strokeWidth={3}/></ComposedChart></ResponsiveContainer></ChartCard>
      </section>

      <section className="grid gap-3 xl:grid-cols-3"><ChartCard title="Perfil dos hóspedes"><SimpleBar rows={model.profiles}/></ChartCard><ChartCard title="Faixa etária"><SimpleBar rows={model.ages}/><div className="mt-1 text-right text-[10px] text-muted-foreground">Cobertura de idade: {pct(model.ageCoverage)}</div></ChartCard><ChartCard title="Filhos"><SimpleBar rows={model.children}/></ChartCard></section>

      <section className="grid gap-3 xl:grid-cols-2">
        <ChartCard title="Gasto com produtos por comprador"><div className="overflow-x-auto"><table className="w-full text-xs"><thead><tr className="border-b text-left text-muted-foreground"><th className="py-2">Comprador</th><th>Compras</th><th>Total</th><th>Mediana</th><th>Média aparada</th></tr></thead><tbody>{model.productSegments.map(r=><tr key={r.name} className="border-b last:border-0"><td className="py-2 font-bold">{r.name}</td><td>{r.compras}</td><td>{fmtBRL(r.total)}</td><td>{fmtBRL(r.mediana)}</td><td>{fmtBRL(r.mediaAparada)}</td></tr>)}</tbody></table></div></ChartCard>
        <ChartCard title="Resultado operacional"><div className="grid grid-cols-2 gap-2 sm:grid-cols-3"><Mini title="Receita" value={fmtBRL(model.kpis.totalRevenue)}/><Mini title="Despesas registradas" value={fmtBRL(model.recordedExpenses)}/><Mini title="GOP registrado" value={fmtBRL(model.actualGop)}/><Mini title="Custos estimados" value={fmtBRL(model.projectedExpenses)}/><Mini title="GOP estimado" value={fmtBRL(model.projectedGop)}/><Mini title="Cancelamento antecipado" value={`${pct(model.cancellationRate)} · ${model.cancellationEligible} reservas`}/></div><details className="mt-3 rounded-xl border border-border bg-muted/30 px-3 py-2 text-[11px] text-muted-foreground"><summary className="cursor-pointer font-bold text-foreground">Premissas provisórias</summary><p className="mt-2">Aluguel: 20% da receita. Salários: 4 × R$ 1.700 + 1 × R$ 1.900/mês. Folguistas: R$ 500/semana. Padaria: R$ 2.050/mês. As despesas reais continuam separadas para não mascarar o resultado.</p></details></ChartCard>
      </section>
      <section className="grid gap-3 xl:grid-cols-2"><RoomTable title="Quartos com mais hospedagens" rooms={model.topRooms}/><RoomTable title="Quartos com pouco ou nenhum uso" rooms={model.lowRooms}/></section>
    </main>
  );
}
...
function SimpleBar({ rows }: { rows:{name:string;value:number}[] }) { return <ResponsiveContainer width="100%" height={230}><BarChart data={rows} layout="vertical" margin={{left:18,right:18}}><CartesianGrid stroke={CHART.grid} strokeDasharray="3 3" horizontal={false}/><XAxis type="number" allowDecimals={false} tick={{fontSize:10}}/><YAxis type="category" dataKey="name" width={128} tick={{fontSize:10}}/><Tooltip/><Bar dataKey="value" name="Reservas" fill={CHART.blue}/></BarChart></ResponsiveContainer>; }
